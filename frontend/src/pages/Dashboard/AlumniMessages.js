      ...
      const processedChats = response.data.data.map(chat => {
        ...
        return {
          ...
          otherUser: {
            ...
            userType: otherUser?.userType || 'Student'
          }
        };
      });
            ...
            <div className="overflow-y-auto h-[calc(100vh-64px)]">
              {loading && chats.length === 0 ? (
                ...
                filteredChats.map((chat) => (
                  <motion.div
                    ...
                    className={`p-4 border-b cursor-pointer transition-colors ${
                      selectedChat?.chatId === chat.chatId ? 'bg-blue-50' : 'hover:bg-gray-50'
                    }`}
                ...
                <div className="text-center p-8 text-gray-500">
                  ...
                  <p>{searchTerm ? 'No students match your search' : 'No conversations yet'}</p>
                </div>
              )}
            </div>
          </div>
              ...
              <>
                ...
                <div className="p-4 bg-white border-b flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{selectedChat.otherUser?.userType || 'Student'}</p>
                  </div>
                  {!socketConnected && (
                  ...
                  {loading && messages.length === 0 ? (
                    ...
                    <div className="flex justify-center items-center h-full text-center text-gray-500">
                      <div>
                        ...
                        <p>No messages yet</p>
                        <p className="text-sm">Start a conversation with {selectedChat.otherUser?.firstName || 'this student'}</p>
                      </div>
                    </div>
                  )}
                </div>
                  ...
                  <form onSubmit={handleSendMessage} className="flex space-x-4">
                    ...
                    <motion.button
                      ...
                      className={`text-white px-4 py-2 rounded-lg flex items-center space-x-2 transition-colors ${
                        message.trim() && !error ? 'bg-blue-500 hover:bg-blue-600' : 'bg-gray-300 cursor-not-allowed'
                      }`}
